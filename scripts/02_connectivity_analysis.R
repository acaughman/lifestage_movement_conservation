...
for (i in 1:nrow(move_combos)) {
  ...
  IS_a_fish <- mean(f_a_fish * fp)
  IS_l_fish <- mean(f_l_fish * fp)
  IS_a_mpa <- mean(f_a_mpa * fp)
  IS_l_mpa <- mean(f_l_mpa * fp)
  ...
  ID_a_fish <- mean((f_a_fish * fp)^(10^-10))
  ID_l_fish <- mean((f_l_fish * fp)^(10^-10))
  ID_a_mpa <- mean((f_a_mpa * fp)^(10^-10))
  ID_l_mpa <- mean((f_l_mpa * fp)^(10^-10))
