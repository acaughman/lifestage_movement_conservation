...
ggplot(eq_pop_size %>% filter(mpa_spacing == 0)) +
  geom_point(aes(movement, mean_pop)) +
...
ggplot(eq_pop_size %>% filter(mpa_spacing == 8)) +
  geom_point(aes(movement, mean_pop)) +
  facet_grid(fp~eggslabeller = label_both) +
  theme_bw() +
  theme(strip.text = element_text(face = "bold"),
        strip.background = element_rect(fill = "white")) +
  labs(x = "Relative Movement",
...
p1 <- ggplot(mpa_connect) +
  ...
  geom_point(aes(movement, relative_mpa_R, color = "Retention", shape = mpa_spacing), size = 5) +
