...
for (fishing_pressure in c(calc_fmsy(10, 0.7) - 0.3, calc_fmsy(10, 0.7), calc_fmsy(10, 0.7) + 0.3)) {
  if (fishing_pressure == calc_fmsy(10, 0.7) - 0.3) {
    fishing_pressure_c <- "low"
  } else if (fishing_pressure == calc_fmsy(10, 0.7)) {
    fishing_pressure_c <- "med"
  } else if (fishing_pressure == calc_fmsy(10, 0.7) + 0.3) {
  ...
  # fish variables
  num_eggs <- 10 # number of eggs per female fish
  n_mort <- 0.7 # natural mortality
  h <- 0.7 # density dependence strength

  dd <- (5 * h - 1) / (4 * h * num_eggs) # density dependence for larval mortality

  f_mort_orig <- array(fishing_pressure, c(resolution, sexes)) # fishing mortality (same dimension as simulation)
  ...
  write_csv(output_df, here::here("outputs", paste0("4x4_0_", fishing_pressure_c, "F.csv")))
